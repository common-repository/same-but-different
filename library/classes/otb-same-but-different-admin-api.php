...
class OTB_Same_But_Different_Admin_API {
	...
	public function display_field ( $data = array(), $post = false, $echo = true ) {
		...
		$field_class = 'otb-form-control otb-form-control-' . $field['type'] . ' ' . $field['id'] . ' ' . $field['class'];
		...
		switch( $field['type'] ) {
			...
			case 'url':
			case 'email':
				$html .= '<input id="' . esc_attr( $field['id'] ) . '" type="text" name="' . esc_attr( $option_name ) . '" placeholder="' . esc_attr( $field['placeholder'] ) . '" value="' . esc_attr( $data ) . '" class="' . esc_attr( $field_class ) . '" />' . "\n";
			break;
			...
			case 'hidden':
				...
				$html .= '<input id="' . esc_attr( $field['id'] ) . '" type="' . esc_attr( $field['type'] ) . '" name="' . esc_attr( $option_name ) . '" placeholder="' . esc_attr( $field['placeholder'] ) . '" value="' . esc_attr( $data ) . '"' . $min . '' . $max . ' class="' . esc_attr( $field_class ) . '" />' . "\n";
			break;
			...
			case 'textarea':
				$html .= '<textarea id="' . esc_attr( $field['id'] ) . '" rows="5" cols="50" name="' . esc_attr( $option_name ) . '" placeholder="' . esc_attr( $field['placeholder'] ) . '" class="' . esc_attr( $field_class ) . '">' . $data . '</textarea><br/>'. "\n";
			break;
			...
			case 'checkbox':
				...
				$html .= '<input id="' . esc_attr( $field['id'] ) . '" type="' . esc_attr( $field['type'] ) . '" name="' . esc_attr( $option_name ) . '" ' . $checked . ' class="' . esc_attr( $field_class ) . '" />' . "\n";
			break;
			...
			case 'checkbox_multi':
				foreach ( $field['options'] as $k => $v ) {
					...
					$html .= '<label for="' . esc_attr( $field['id'] . '_' . $k ) . '" class="checkbox_multi"><input type="checkbox" ' . checked( $checked, true, false ) . ' name="' . esc_attr( $option_name ) . '[]" value="' . esc_attr( $k ) . '" id="' . esc_attr( $field['id'] . '_' . $k ) . '" class="' . esc_attr( $field_class ) . '" /> ' . $v . '</label><br />';
				}
			break;
			...
			case 'radio':
				foreach ( $field['options'] as $k => $v ) {
					...
					$html .= '<label for="' . esc_attr( $field['id'] . '_' . $k ) . '"><input type="radio" ' . checked( $checked, true, false ) . ' name="' . esc_attr( $option_name ) . '" value="' . esc_attr( $k ) . '" id="' . esc_attr( $field['id'] . '_' . $k ) . '" class="' . esc_attr( $field_class ) . '" /> ' . $v . '</label> ';
				}
			break;

			case 'select':
				$html .= '<select name="' . esc_attr( $option_name ) . '" id="' . esc_attr( $field['id'] ) . '" class="' . esc_attr( $field_class ) . '">';
				foreach ( $field['options'] as $k => $v ) {
					$selected = false;
					if ( $k == $data ) {
						$selected = true;
			...
			case 'select_multi':
				$html .= '<select name="' . esc_attr( $option_name ) . '[]" id="' . esc_attr( $field['id'] ) . '" multiple="multiple" class="' . esc_attr( $field_class ) . '">';
				foreach ( $field['options'] as $k => $v ) {
					$selected = false;
					if ( in_array( $k, $data ) ) {
						$selected = true;
		...
		switch( $field['type'] ) {
			case 'range':
				...
				$html .= '<input id="' . esc_attr( $field['id'] ) . '" type="range" min="' . esc_attr( $field['input_attrs']['min'] ) . '" max="' . esc_attr( $field['input_attrs']['max'] ) . '" step="' . esc_attr( $field['input_attrs']['step'] ) . '" name="' . esc_attr( $option_name ) . '" value="' . esc_attr( $data ) . '" class="' . esc_attr( $field_class ) . '" />' . "\n";
